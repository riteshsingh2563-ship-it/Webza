...
for (let i = 0; i < 15; i++) { const d = document.createElement('span'); d.className = 'ydot'; yearGrid.appendChild(d); }
(function marquee(){
  const track = $('#mqTrack');
...
ScrollTrigger.create({trigger:'#mathCard', start:'top 72%', once:true, onEnter: () => {
  ...
  gsap.to($$('#yearGrid .ydot').slice(0, years), {
    backgroundColor:'#6B7D50', borderColor:'rgba(107,125,80,.7)',
...
(function heroFilm(){
  ...
  document.querySelectorAll('#yearGrid .ydot').forEach((el, i) => { if (i < years) el.style.background = '#6B7D50'; });
